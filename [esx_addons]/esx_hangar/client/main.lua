...
RegisterNetEvent('esx_hangar:closemenu')
AddEventHandler('esx_hangar:closemenu', function()
    menuIsShowed = false
...
RegisterNUICallback('escape', function(data, cb)
    TriggerEvent('esx_hangar:closemenu')
    cb('ok')
end)
...
RegisterNUICallback('spawnVehicle', function(data, cb)
    ...
    if thisGarage then
        ...
        if ESX.Game.IsSpawnPointClear(spawnCoords, 2.5) then
            thisGarage = nil
            TriggerServerEvent('esx_hangar:updateOwnedVehicle', false, nil, nil, data, spawnCoords)
            TriggerEvent('esx_hangar:closemenu')
    ...
    elseif thisPound then
        ...
        ESX.TriggerServerCallback('esx_hangar:checkMoney', function(hasMoney)
            if hasMoney then
                if ESX.Game.IsSpawnPointClear(spawnCoords, 2.5) then
                    TriggerServerEvent('esx_hangar:payPound', data.exitVehicleCost)
                    thisPound = nil
...
RegisterNUICallback('impound', function(data, cb)
    ...
    TriggerServerEvent('esx_hangar:setImpound', data.poundName, data.vehicleProps)
    TriggerEvent('esx_hangar:closemenu')
...
AddEventHandler('esx_hangar:hasEnteredMarker', function(name, part)
...
AddEventHandler('esx_hangar:hasExitedMarker', function()
    thisGarage = nil
    thisPound = nil
    ESX.HideUI()
    TriggerEvent('esx_hangar:closemenu')
        ...
        if nearMarker then
                ...
                if (#(coords - vector3(v.EntryPoint.x, v.EntryPoint.y, v.EntryPoint.z)) <
                    ...
                    if not isInVehicle then
                        if IsControlJustReleased(0, 38) and not menuIsShowed then
                            ESX.TriggerServerCallback('esx_hangar:getVehiclesInParking', function(vehicles)
                                if next(vehicles) then
                                    ...
                                    ESX.TriggerServerCallback('esx_hangar:getVehiclesImpounded', function(vehicles)
                                        if next(vehicles) then
                    ...
                    if isInVehicle then
                        if IsControlJustReleased(0, 38) then
                            ...
                            ESX.TriggerServerCallback('esx_hangar:checkVehicleOwner', function(owner)
                                if owner then
                                    ESX.Game.DeleteVehicle(vehicle)
                                    TriggerServerEvent('esx_hangar:updateOwnedVehicle', true, currentMarker, nil,
            ...
            for k, v in pairs(Config.Impounds) do
                ...
                if (#(coords - vector3(v.GetOutPoint.x, v.GetOutPoint.y, v.GetOutPoint.z)) < 2.0) then
                    ...
                    if IsControlJustReleased(0, 38) and not menuIsShowed then
                        ESX.TriggerServerCallback('esx_hangar:getVehiclesInPound', function(vehicles)
                            if next(vehicles) then
                                menuIsShowed = true
            ...
            if isInMarker and not HasAlreadyEnteredMarker or
                ...
                if LastMarker ~= currentMarker or LastPart ~= currentPart then
                    TriggerEvent('esx_hangar:hasExitedMarker')
                end
                ...
                TriggerEvent('esx_hangar:hasEnteredMarker', currentMarker, currentPart)
            end
