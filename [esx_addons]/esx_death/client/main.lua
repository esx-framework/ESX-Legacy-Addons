...
local function remaining()
    ...
    local early = math.max(0, (state.earlyRemaining or 0) - elapsed)
    local total = math.max(0, (state.remaining or 0) - elapsed)
    local distress = math.max(0, (state.distressRemaining or 0) - elapsed)

    return early, total, distress
end
...
local function deathCauseLabel(cause)
    ...
    if not cause or cause == 0 then
        return Translate('death_cause_unknown')
    end

    if cause < 0 then
        cause = cause + 4294967296
    end
...
local function deathReason()
    if not Config.ShowDeathReason then
        return nil
    end
    ...
    if info.killedByPlayer then
        if Config.ShowKillerName and info.killerServerId then
            ...
            if name and name ~= '' then
                return Translate('killed_by', name)
            end
        end
...
local function updateDeathInfo(data)
    ...
    if data.killedByPlayer ~= nil then
        deathInfo.killedByPlayer = data.killedByPlayer
    end

    if data.killerServerId ~= nil then
        deathInfo.killerServerId = data.killerServerId
    end

    if data.deathCause ~= nil then
        deathInfo.deathCause = data.deathCause
    end
end
...
local function refreshUI()
    if not uiReady or not dead then
        return
    end
    ...
    send('state', {
        ...
        total = state.total,
        distressRemaining = math.ceil(distress),
        distressSent = state.distressSent,
        fine = state.fine,
        pending = pending,
        distressPending = distressPending,
...
local function enterDeath()
    ...
    CreateThread(function()
        ...
        if Config.DeathAnim.enabled then
            ...
            dbg('deathThread:resurrect-for-death-anim', {
                x = coords.x,
                y = coords.y,
                z = coords.z,
            ...
            dbg('deathThread:anim-dict-result', {
                loaded = HasAnimDictLoaded(anim.dict),
                dict = anim.dict,
                dead = dead,
        ...
        while dead and generation == cycle do
            ...
            if Config.CameraEnabled and not cursor then
                ProcessCamControls()
            end

            if IsDisabledControlJustReleased(0, 289) then
                setCursor(not cursor) -- F2
            end
            ...
            elseif IsDisabledControlPressed(0, 38) and early <= 0 and not pending then
                ...
                if progress >= 1 then
                    dbg('input:E-hold-complete-respawn', {
                        early = early,
                        progress = progress,
                        pending = pending,
                        dead = dead
            ...
            if Config.DeathAnim.enabled then
                ...
                if HasAnimDictLoaded(anim.dict)
                    and not IsEntityPlayingAnim(ped, anim.dict, anim.name, 3)
                    and not IsPedInAnyVehicle(ped, false)
                then
                    TaskPlayAnim(
                        ped, anim.dict, anim.name, anim.fadeIn, anim.fadeOut, -1,
                        anim.flags, anim.playbackRate, false, false, false
                    )
                end
            end
...
-- Lifecycle: state application and recovery
...
local function recover(data)
    ...
    if thisSession ~= session or not ESX.PlayerLoaded then
        recovering = false
        DoScreenFadeIn(500)
        dbg('recover:aborted-session-or-not-loaded', {
    ...
    dbg('recover:resurrecting', {
        ped = ped,
        x = coords.x,
        y = coords.y,
        z = coords.z,
        heading = heading,
        hasPoint = data.point ~= nil
    })
    ...
    if data.loadout then
        ESX.SetPlayerData('loadout', data.loadout)
    end
...
requestRespawn = function()
    ...
    if not dead or pending or GetGameTimer() < retryAt then
        dbg('requestRespawn:blocked-initial', {
            dead = dead,
            pending = pending,
            retryAt = retryAt,
            now = GetGameTimer()
    ...
    xLib.callback('esx_death:respawn', false, function(result)
        dbg('requestRespawn:server-response', {
            request = request,
        ...
        if result and result.error == 'not_dead' then
            syncState()
        end
    end)
end
...
AddEventHandler('esx_death:requestDistress', function()
    ...
    xLib.callback('esx_death:distress', false, function(result)
        dbg('distress:server-response', {
            request = request,
            currentRequest = distressId,
            dead = dead,
            result = result
        })

        if not dead or request ~= distressId then
            return
        end
...
syncState = function()
    ...
    if dead and GetGameTimer() - syncedAt < 2000 then
        return
    end
...
AddEventHandler('esx:onPlayerDeath', function(data)
    ...
    state = {
        dead = true,
        earlyRemaining = math.ceil(Config.EarlyRespawnTimer / 1000),
        remaining = total,
        total = total,
        distressRemaining = 0,
...
RegisterNetEvent('esx_ambulancejob:clsearch', function(medicId)
    ...
    if source ~= 65535 or not dead then
        return
    end
    ...
    local ped = PlayerPedId()
    local medic = GetPlayerPed(player)

    if #(GetEntityCoords(ped) - GetEntityCoords(medic)) > 4.0 then
        return
    end
    ...
    if dead and cycle == generation then
        ClearPedTasksImmediately(ped)
    end
end)
...
exports('GetDeathState', function()
    ...
    return {
        dead = dead,
        earlyRemaining = early,
        remaining = total,
        distressRemaining = distress
    }
end)
