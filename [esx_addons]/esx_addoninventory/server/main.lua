...
local function getInventory(name, owner)
	...
	-- 		end
	-- 	end
	local existingInventory = Inventories[name]?[owner]
	if existingInventory then return existingInventory end

	local dbInventory, dbItems = Database.fetchInventory(name, false), Database.fetchInventoryItems(name, owner)
	if not dbInventory or not dbItems then return end

	if not Inventories[name] then
		Inventories[name] = {}
	end

	Inventories[name][owner] = CreateAddonInventory(name, owner, dbItems)
...
local function getSharedInventory(name)
	local existingInventory = SharedInventories[name]
	if existingInventory then return existingInventory end

	local dbAccount, dbItems = Database.fetchInventory(name, true), Database.fetchInventoryItems(name)
	if not dbAccount or not dbItems then return end

	SharedInventories[name] = CreateAddonInventory(name, nil, dbItems)
