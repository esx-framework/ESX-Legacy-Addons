...
local ARREST_DICT, ARREST_ANIM = 'mp_arresting', 'idle'
dragStatus.isDragged, isInShopMenu = false, false

local function playCuffAnim(ped)
    RequestAnimDict(ARREST_DICT)
    local tries = 0
    while not HasAnimDictLoaded(ARREST_DICT) and tries < 100 do
        Wait(10); tries = tries + 1
    end
    TaskPlayAnim(ped, ARREST_DICT, ARREST_ANIM, 8.0, -8.0, -1, 49, 0.0, false, false, false)
end
local function stopCuffAnim(ped)
    ClearPedSecondaryTask(ped)
    if HasAnimDictLoaded(ARREST_DICT) then
        RemoveAnimDict(ARREST_DICT)
    end
end
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
    if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' and dispatchNumber == 'police' then
...
AddEventHandler('esx_policejob:hasExitedEntityZone', function(entity)
    if CurrentAction == 'remove_entity' and CurrentActionData and CurrentActionData.entity == entity then
        CurrentAction = nil
    end
end)
...
CreateThread(function()
    while true do
        if isHandcuffed and (not Config.Cuffs or not Config.Cuffs.FreezePlayer) then
            ...
            if IsEntityPlayingAnim(ped, ARREST_DICT, ARREST_ANIM, 3) ~= 1 then
                playCuffAnim(ped)
            end
...
CreateThread(function()
    while true do
        local sleep = 1000
        if isHandcuffed and Config.Cuffs and Config.Cuffs.FreezePlayer then
            sleep = 0
            ...
            DisableControlAction(0, 0,   true) -- View
            DisableControlAction(0, 26,  true) -- Look behind
            DisableControlAction(0, 73,  true) -- Clear anim
            DisableControlAction(2, 199, true) -- Pause
            DisableControlAction(0, 59,  true) -- Steer in vehicle
            DisableControlAction(0, 71,  true) -- Forward in vehicle
            DisableControlAction(0, 72,  true) -- Reverse in vehicle
            DisableControlAction(2, 36,  true) -- Stealth
            DisableControlAction(0, 47,  true) -- Weapon
...
ESX.RegisterInput("police:interact", "(ESX PoliceJob) " .. TranslateCap('interaction'), "keyboard", "E", function()
    ...
    elseif CurrentAction == 'remove_entity' then
        if CurrentActionData and CurrentActionData.entity and DoesEntityExist(CurrentActionData.entity) then
