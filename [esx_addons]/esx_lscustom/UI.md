...
Brand, background, secondary and accent colors, plus the header logo, follow
`xLib.colors.getESXTheme()` (`esx:ui:*` convars) when the workshop opens.
...
- `assets/esx-logo.png` is an unchanged copy of
  `esx_skin/web/src/assets/icons/logo-esx.png`, shown when `esx:ui:logoUrl` is empty.
